...
ui <- fluidPage(
  theme = bslib::bs_theme(bg = "#FFFFFF", fg = "#212529", primary = "#007BFF"),
  ...
  div(class = "main-container",
      ...
      div(class = "title-container",
          div(class = "main-title", "Estimates of MP Positions"),
          div(class = "sub-title", "Royal Holloway / Survation / UK in a Changing Europe")
      ),
  ...
  tags$footer(
    div(
      img(src = 'https://www.royalholloway.ac.uk/images/logo-og-default.jpg', width = "100", style = "margin-right: 15px; vertical-align: middle;"),
      img(src = 'https://cdn.survation.com/wp-content/theme/images/logo.png', width = "100", style = "margin-left: 15px; vertical-align: middle;"),
      img(src = 'https://www.whatukthinks.org/eu/wp-content/uploads/2020/12/UKinEur-blue-letters-e1607514355509.png', width = "100", style = "margin-left: 15px; vertical-align: middle;"),
      style = "position: fixed;
               bottom: 0;
               width: 100%;
               height: 80px; /* Adjusted to allow for padding */
               text-align: center;
...
server <- function(input, output, session) {
  ...
  observeEvent(input$mp_a, {
    if (!is.null(input$mp_a) && input$mp_a != "") {
      updated_choices <- setdiff(unique(dat$DisplayName), input$mp_a)
    } else { 
      updated_choices <- unique(dat$DisplayName) 
    }
    updateSelectizeInput(session, "mp_b", choices = updated_choices)
  })
...
"
  ...
  output$party_comparison <- renderUI({
    ...
    party_prefix <- case_when(party == "Plaid Cymru" ~ "",
                              TRUE ~ "the")
    party_postfix <- case_when(party %in% c("Plaid Cymru",
...
<p>&nbsp;</p>
<ul>
<li> <em>{MP_A_full}</em> is <strong>{wep}</strong> {direction} of the average (median) MP in {party_prefix} {party} {party_postfix} </li>
  ...
  output$party_comparison_plot <- renderPlot({
    ...
    plot_df <- theta_mp_party() |>
      mutate(party_label = case_when(
        Party %in% c("Conservative", "Green", "Labour", "Scottish National") ~ paste(Party, "Party"),
        TRUE ~ Party)) |> 
